...
const SignUp = () => {
  ...
  return (
    <div className={`flex border-2 border-red-500 px-40 h-screen bg-gradient-to-r from-blue-100 to-green-200 transition duration-500 ease-in-out ${fadeIn ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className={`w-2/3 p-6 px-10 text-blackGreen ${fadeIn ? 'opacity-100' : 'opacity-0'}`}>
        <img src={logo} className='w-28 mb-8' alt="Logo" />
          ...
          <form className="space-y-1 onSubmit={formik.handleSubmit}">
            <div className="flex gap-4">
              ...
              <fieldset className="w-1/2 border border-blue-400 rounded-xl p-1 px-2">
                ...
                <input id="lastName"
                  className={`w-full bg-transparent text-sm text-gray-800 outline-none ${formik.touched.lastName && formik.errors.lastName ? 'border-red-500' : ''}`}
                  placeholder="Harrison"
                  {...formik.getFieldProps('lastName')}
                />
                {formik.touched.lastName && formik.errors.lastName && <p className="text-xs text-red-500">{formik.errors.lastName}</p>}
              </fieldset>
            </div>
            ...
            <div className="flex gap-4">
              ...
              <fieldset className="w-1/2 border border-blue-400 rounded-xl px-2">
                ...
                <input id="phoneNumber"
                      ...
                      inputMode="numeric"
                      pattern="[0-9]*"
                      className={`w-full bg-transparent text-sm text-gray-800 outline-none ${formik.touched.phoneNumber && formik.errors.phoneNumber ? 'border-red-500' : ''}`}
                      placeholder="(+234) -8100000336"
                  {...formik.getFieldProps('phoneNumber')}
                />
                {formik.touched.phoneNumber && formik.errors.phoneNumber && <p className="text-xs text-red-500">{formik.errors.phoneNumber}</p>}
              </fieldset>
            </div>
